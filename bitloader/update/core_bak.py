...
def process_addr_tx(addr_info, tx_set, sent_set):
...
def update_address_tx(pk_id, tx_set, pk_sent):
    ...
    if not Config.DEBUG:
        res = mongodb_cli.get_last_one(table=Config.address_tx, id=pk_id, args={"pubkey_id": pk_id}, order="shard")
        ...
        if size + len(tx_list) <= Config.SHARDING_SIZE:  
            ...
            mongodb_cli.push_to_list(table=Config.address_tx, id=pk_id, cond=cond, push_list={"tx_list": tx_list, "sent_list":sent_list}, setting=set_size)
            ...
            mongodb_cli.push_to_list(table=Config.address_tx, id=pk_id, cond=cond, push_list={"tx_list": tx_list[:n], "sent_list":sent_list[:n]}, setting=set_size)
...
def update_tx_value(tx_value):
    ...
    for tx_id in tx_value:
        ...
        if not Config.DEBUG:
            mongodb_cli.update(table=Config.tx_value, id=tx_id, query={'tx_id':tx_id}, args=tx_info)
            

def process_addr_info(addr_info):
...
def update_address_info(pk_id, pk_info):
    ...
    record = mongodb_cli.get(table=Config.address_info, id=pk_id, args={'pubkey_id': pk_id})
    ...
    if not Config.DEBUG:
        mongodb_cli.update(table=Config.address_info, id=pk_id, query={'pubkey_id':pk_id}, args=pk_info)
...
def process_tx(block_id):
    ...
    process_addr_info(addr_info, tx_set)
    ...
    process_addr_tx(addr_info, tx_set, sent_set)
