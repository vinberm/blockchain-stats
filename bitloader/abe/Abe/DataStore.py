...
from relay import get_relay, notify_block
...
#    def _is_descended_from(store, block_id, ancestor_id):
    ...
    def import_block(store, b, chain_ids=None, chain=None, size=0):
        ...
        for pos in xrange(len(b['transactions'])):
            ...
            if pos == 0:
                value, script = tx['value_out'],  tx['scriptSig']
        ...
        ##import block relay (id, relay, size ,fee, ntime)
        # block_id, size,  store.intin(b['nTime'])        
        fee = int(value) - (5000000000L / (2**(b['height'] / 210000)))
        relay = get_relay(script)
        store.sql("""
                INSERT INTO block_relay
                    (block_id, relay, size, ntime, block_fee)
                VALUES (?, ?, ?, ?, ?)""",
                      (block_id, relay, size, store.intin(b['nTime']),fee))
    ...
    def import_tx(store, tx, is_coinbase, chain):
        ...
        for pos in xrange(len(tx['txOut'])):
            ...
            if is_coinbase:
                tx['scriptPubkey'] = txout['scriptPubKey']
        ...
        for pos in xrange(len(tx['txIn'])):
            ...
            if is_coinbase:
                txout_id = None
                tx['scriptSig'] = store.binin(txin['scriptSig'])
    ...
    def _offer_block_to_chain(store, b, chain_id):
        ...
        print 'orphan', b['block_id'], in_longest
        if in_longest > 0:
            store.sql("""
                UPDATE chain
        ...
        store.sql("""
            UPDATE chain_candidate
             ...
             WHERE block_id = ? AND chain_id = ?""",
                  (block_id, chain_id))
        print 'disconnect_block', block_id, 1
    ...
    def connect_block(store, block_id, chain_id):
        store.sql("""
            UPDATE chain_candidate
             ...
             WHERE block_id = ? AND chain_id = ?""",
                  (block_id, chain_id))
        print 'connect_block', block_id, 1
    ...
    def import_blkdat(store, dircfg, ds, filename="[unknown]"):
        ...
        while filenum == dircfg['blkfile_number']:
            ...
            if not store.offer_existing_block(hash, chain.id):
                ...
                store.import_block(b, chain = chain, size=int(length))
